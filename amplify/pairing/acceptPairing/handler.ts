 ...
 * the caller's (client-supplied) email, then add the caller's sub to `members`
 * and flip it to ACTIVE. This is the privileged step the invitee couldn't do
 * directly (they weren't yet an owner of the multi-owner row).
 */
...
export const handler: Schema['acceptInvite']['functionHandler'] = async (event) => {
  ...
  const email = normalizeEmail(event.arguments.callerEmail);
