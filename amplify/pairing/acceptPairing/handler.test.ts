...
type Ev = { arguments: { pairingId: string }; identity: unknown };
const call = handler as unknown as (ev: Ev) => Promise<unknown>;
const identity = { sub: 'u2', claims: { sub: 'u2', email: 'b@x.com' } };
const pending = {
  id: 'p1',
  members: ['u1'],
...
describe('acceptPairing handler', () => {
  ...
  it('activates a pairing addressed to the caller', async () => {
    ...
    const out = await call({ arguments: { pairingId: 'p1' }, identity });
  ...
  it('throws when the pairing does not exist', async () => {
    ...
    await expect(call({ arguments: { pairingId: 'x' }, identity })).rejects.toThrow('not found');
  });
  ...
  it('rejects when the invite is addressed to a different email', async () => {
    ...
    await expect(call({ arguments: { pairingId: 'p1' }, identity })).rejects.toThrow(
      'different email',
    );
